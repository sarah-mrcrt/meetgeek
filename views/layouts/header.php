<nav id="ma_navbar" class="navbar fixed-top navbar-expand-sm navbar-dark">
    <div class="container">
        <a class="navbar-brand" href="index.php?action=home">Cinematic</a>

        <!-- Menu burger -->
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent"
            aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>
        <!-- Menu PC -->
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav ms-auto">
                <li class="nav-item">
                    <a class="nav-link" href="index.php?action=home">Home</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="index.php?action=about">About</a>
                </li>
                <!-- DROPDOWN COMPONENT -->
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button"
                        data-bs-toggle="dropdown" aria-expanded="false">Missions</a>
                    <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                        <li><a class="dropdown-item" href="#">Humans in Space</a></li>
                        <li><a class="dropdown-item" href="#">Space</a></li>
                        <li><a class="dropdown-item" href="#">Earth</a></li>
                    </ul>
                </li>
                <!-- end DROPDOWN COMPONENT -->
                <li class="nav-item">
                    <a class="nav-link" href="#contact">Contact</a>
                </li>
            </ul>
        </div>

        <!-- Réseaux sociaux -->
        <div>
            <a href="#!" role="button"><i class="fab fa-facebook-f"></i></a>
            <a href="#!" role="button"><i class="fab fa-instagram"></i></a>
            <a href="#!" role="button"><i class="fab fa-twitter"></i></a>
            <a href="#!" role="button"><i class="fab fa-discord"></i></a>
        </div>
    </div>
</nav>


<header>
    <?php
            if (isset($_SESSION['id'])) {
            ?>
    <form action='index.php?action=recherche' method='POST'>
        <input type='search' placeholder='Rechercher des amis' name='recherche'>
        <input type='submit' placeholder='Rechercher'>
    </form>

    <?php
                echo "
                    <a href='index.php?action=mur&id=".$_SESSION['id']."'>" . $_SESSION['login'] ."</a>";
                ?>

    <!-- <a href="index.php?action=mur&id=<?php $_SESSION['id']?>"><?php $_SESSION['login']?></a> -->


    <a href='index.php?action=accueil'>Accueil</a>
    <a href='index.php?action=gestion-ami'>Mes amis</a>
    <a href='index.php?action=deconnexion'>Déconnexion</a>
    <?php
            }
            ?>
</header>